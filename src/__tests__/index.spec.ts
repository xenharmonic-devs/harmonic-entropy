...
describe('Harmonic entropy calculator (class)', () => {
  ...
  it('can be serialized', () => {
    ...
    const entropy = new EntropyCalculator(options);
    expect(entropy.ofCents(100)).toBe(2.957849276572092); // Inaccurate due to low resolution artifacts
    const expectedLegacy =
      '{"type":"EntropyCalculator","options":{"N":1000,"res":100},"tableY":[1.0406308287245427,2.957849276572092,2.8025290882335474,2.7199329429047627,2.679974909320833,2.5697430947435373,2.7927280401803567,2.3836319835863327,2.762987388249189,2.6175492942407934,2.7652522430326556,2.896296296715723,1.6851019186072065,2.89058975083966,2.7231137316080005,2.731079581152761,2.5822264414027614,2.759003908365329,2.8244326764802286,1.9969801938222762,2.856602496189162,2.7058764103492132,2.6011157743857187,2.7912880810361584,2.1317612578877587]}';
    const serialized = JSON.stringify(entropy);
    if (serialized !== expectedLegacy) {
      expect(serialized).toBe(
        '{"type":"EntropyCalculator","options":{"N":1000,"res":100},"tableY":[1.0406308287245427,2.957849276572092,2.8025290882335474,2.7199329429047627,2.679974909320833,2.5697430947435373,2.7927280401803567,2.3836342040321985,2.762987388249189,2.6175492942407934,2.7652544634785214,2.896296296715723,1.6851030288301392,2.890591971285527,2.723112621385067,2.731079581152761,2.5822264414027614,2.759003908365329,2.8244326764802286,1.9969801938222762,2.856604716635028,2.705878630795079,2.6011157743857187,2.7912880810361584,2.1317612578877587]}',
      );
    }
  });
  ...
  it('can be deserialized', () => {
    ...
    expect(entropy.ofCents(100)).toBe(2.957849276572092); // Should match the above test.
